...
class CreditSystem extends Component {
	...
	render() {
        ...
        <div ref={ref => this.barChartBig = ref}></div>
	...
	initBar () {
		...
		let svg = d3.select(me.barChartBig)
			.append('svg')
		...
		this._createBarChart(path_g, w, h, diffy, diffx, '#0a3352', '#0b4063') // 背景柱图
		// 实体填充
		let sumArr = [];
		data.map((t) => {
			//console.log(t)
		...
		data.map((t, i) => {
			for(var j = 0; j < arr[0].length; j++) {
				...
				// console.log(arr[i][j])
				this._createBarChart(d3.select('.path_g' + i), arr[i][j], h, diffy, diffx, 'url(#myGradient' + j + ')', startColor[j], true, sum);
			}
		});
	}
	// 柱图创建
