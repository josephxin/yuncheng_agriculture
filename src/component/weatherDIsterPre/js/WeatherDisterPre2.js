...
class WeatherDisterPre2 extends React.Component {
	constructor(props) {
		super();
		this.rootStyle = {
			position: 'absolute',
	...
	_createDom(data) {
		...
		if(data.length > 0) {
			data.map((item, index) => {
	          ...
	          <div className="qxzhjcyj-box">
	            <div className="qxzhjcyj-wrap">
	              <span className="qxzhjcyj-type">{typeObj[typeshowName]}</span>
	              <i className={stateClass}></i>
	              <span className="qxzhjcyj-state">{item.region} {stateText}</span>
	              <span className={`qxzhjcyj-grade ${item.grade}`}>{grade + typeObj[typeshowName]}</span>
	              <span>预警</span>
	              <span className="qxzhjcyj-time">{time}</span>
	            </div>
	          </div>
	        </li>
				)
			...
			for(var i = 0; i < 1; i++) {
				domList.push(
					<li className="qxzhjcyj-li" key={i}>
			    	<div className="qxzhjcyj-box" style= {{marginLeft:'15px'}}>
			    		<div className="qxzhjcyj-wrap">
			    			<span className="qxzhjcyj-type"></span>
			    			<i className=""></i>
			    			<span className="qxzhjcyj-state"></span>
			    			<span className="qxzhjcyj-grade yellow">暂无预警信息</span>
			    			<span></span>
			    			<span className="qxzhjcyj-type" style= {{left:'100%'}}>{hours-i}时</span>
			    		</div>
			    	</div>
			    </li>
				);
...
export default WeatherDisterPre2