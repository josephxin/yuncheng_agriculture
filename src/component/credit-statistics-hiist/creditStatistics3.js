...
class creditStatistics3 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    me._option = {
      ...
      tooltip: {
        ...
        formatter: function (d) {
          ...
          d.forEach((s, i) => {
            if (i === 0) {
              str = [`<span style="font-size: 16px;">${s.name}</span>`, '</br>', s.marker, s.seriesName, ': ', s.value+'人' || '--'].join('');
            } else {
              str += ['</br>', s.marker, s.seriesName, ': ', s.value+'人' || '--'].join('');
            }
          });
          return str;
        },
...
export default creditStatistics3;