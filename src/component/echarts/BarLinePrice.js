    ...
    this.options = {
      tooltip: {
        ...
        formatter: function (d) {
          ...
          d.forEach((s, i) => {
            if (i === 0) {
              str = [`<span style="font-size: 16px;">${s.name}</span>`, '</br>', s.marker, s.seriesName, ': ', s.value+'元' || '--'].join('');
            } else {
              str += ['</br>', s.marker, s.seriesName, ': ', s.value+'元' || '--'].join('');
            }
          });
          return str;
        },
      ...
      series: [
        {
          ...
          areaStyle: {
            color: {
              ...
              colorStops: [
                {
                  offset: 0, color: 'rgba(59,255,208,.3)'
                },
                {
                  offset: 0.4, color: 'rgba(59,255,208,.3)'
                },
                {
                  offset: 1, color: 'rgba(59,255,208,0)'
                }
              ],
            }
          },
