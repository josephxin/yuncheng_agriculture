...
const lineColor = ['rgba(255,253,4,.9)', 'rgba(255,253,4,.9)'];

class BarLine extends React.Component {
	constructor(props) {
		super(props);
		this.timeWidth = props.timeWidth || 380;
		this.state = {
			index: 0,
			date: []
		};
		...
		this.options = {
			...
			tooltip: {
				...
				formatter: function(d) {
					...
					d.forEach((s, i) => {
						if(i === 0) {
							str = [s.name, '</br>', s.seriesName, ': ', s.value + '万吨' || '--'].join('');
	...
	setData(d) {
		this.lock = true;
		this.scale = scaleLinear().domain([0, d.dataBar.length - this.len]).range([this.len * this.timeWidth / d.dataBar.length, this.timeWidth]);
		this.setState({ ...d
		});
	}
	...
	componentDidUpdate() {
		...
		if(me.props.showBar) {
			...
			options.series[1].name = state.legend[1];
			options.legend.data = [{
				name: state.legend[0],
				icon: 'rect'
			}, {
	...
	createLegend() {
        ...
        <li style={{
          ...
          <div style={{float: 'left'}}>{data[1]}</div>
        </li>
      </ul>
		);
	}
	...
	render() {
        ...
        {/*<div className={'x-legend'} style={{textAlign: 'center'}}>{this.createLegend()}</div>*/}
        ...
        <div className={'time-line'} style={{
          ...
          position: 'absolute',
          top: this.props.timeTop || 215,
          left: this.props.timeLeft || 20,
        }}>
