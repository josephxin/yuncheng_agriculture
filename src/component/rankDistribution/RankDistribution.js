 ...
 * 病虫害上报分析
 * */
class RankDistribution extends Component {
  constructor(props) {
    super(props);
    this.state = {
      pieData: [
  ...
  render() {
    return (
      <div ref={'pieChart'} style={{width: this.props.width, height: this.props.height,marginTop:20}}></div>
    )
  }
  initData(d) {
	    let me = this;
    ...
    me.option = {
      tooltip: {
        formatter(data) {
          let d = data.data;
          if (d.type) {
            return d.type + '</br>' + d.name + '：' + d.value+'家'
          }
