...
class WeifangTurnMap extends React.Component {
  ...
  initMap() {
    ...
    sortData.forEach(function (item, index) {
      ...
      rTotal += parseFloat(item.company);
    });
    ...
    for (let i in me.geoCoord) {
      ...
      mapData.forEach((item, index) => {
        if (item.name.indexOf(i) > -1) {
          ...
          effectScatter.value = me.geoCoord[i];
          scatter.name = i;
          scatter.all = item;
          scatter.value = me.geoCoord[i];
          //json.symbol = 'circle'
          item.geoCoord = me.geoCoord[i];
        }
      ...
      series: [
        ...
        },
        {
          ...
          data: scatterData.map(function (item, i) {
            if (item.symbolCor && item.symbolCor.indexOf('green') > -1) {
              item.symbol = 'image://' + greenScatter;
            }
            if (item.symbolCor && item.symbolCor.indexOf('yellow') > -1) {
